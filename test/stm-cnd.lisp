    ...
    (let ((sfx (stm:? ping (new-state :ping))))
      ...
      (labels ((val (o) (fset:@ o :val))
               (proc-item-10 (o rule)
                 ...
                 (if (> (val o) 5)
                     (stm:cnd/halt-itr rule :val-10 "val is larger than 10" o))
                (ping-list o))
               (proc-item-20 (o rule)
                 (declare (keyword rule))
                 (if (> (val o) 10)
                     (stm:cnd/halt-operation rule :val-20 "val is larger than 20"))
                 (ping-list o)))
        ...
        (is-error (stm:itr/all sfx #'proc-item-20) 'stm:cnd/halt-operation)
        ...
        (stm:mvb (val cnd)
          ...
          (is-type cnd 'stm:cnd/halt-itr)
          (is (stm:cnd/flag cnd) :val-10))

        (stm:mvb (val cnd)
          (stm:mutate! (stm:acc/n sfx 30 #'proc-item-20))
          (is val nil)
          (is-type cnd 'stm:cnd/halt-operation)
          (is (stm:cnd/flag cnd) :val-20))
