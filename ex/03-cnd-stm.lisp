      ...
      (labels ((val (o) (fset:@ o :val))
               (proc-item-10 (o rule)
                 ...
                 (if (> (val o) 10) ; halt when val is large
                     (cnd/halt-itr rule o :val-10 "val is larger than 10" o)
                     (format t "~&; {~a cnd---10}: ~a~&" rule o)) ; print value
                 o)
               (proc-item-20 (o rule)
                 (declare (keyword rule))
                 (if (> (val o) 20) ; discard operation when val is larger
                     (cnd/discard-operation rule o :val-20 "val is larger than 20")
        ...
        (cnd/bind-warn (val cnd)
          ...
          ; {PING cnd---10}: #{| (CNT 4) (VAL 8) (STATE PING) |} ; starts at cnt=3
          ; {PONG cnd---10}: #{| (CNT 5) (VAL 9) (STATE PONG) |}
          ; WARNING: halted itr w/VAL-10 @ {PING}: val is larger than 10
        ...
        (cnd/bind-warn (val cnd)
          ...
          ; WARNING: halted mutation w/VAL-20 @ {PONG}: val is larger than 20
          (print val) ; nil
